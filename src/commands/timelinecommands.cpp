        ...
        foreach (const Track & track, m_model.trackList()) {
            ...
            for (int i = playlist.count() - 1; i >= 0; --i) {
                ...
                if (clip->get_int(kUndoIdProperty) < 0) {
                    ...
                    QModelIndex modelIndex = m_model.createIndex(i, 0, trackIndex);
                    m_model.beginRemoveRows(modelIndex.parent(), i, i);
                    playlist.remove(i);
